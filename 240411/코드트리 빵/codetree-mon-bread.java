...
public class Main {
	...
	private static void move() {
		while (!queue.isEmpty() && queue.peek().t < t) {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (map[ny][nx] < 0 && map[ny][nx] > -t) {
					continue;

				}

				if (ny == person.dest[0] && nx == person.dest[1]) {
					check = true;
	...
	private static int bfs(int y, int x, int[] dest) {
		int[][] dp = new int[n][n];
